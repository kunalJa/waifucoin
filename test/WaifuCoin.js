...
const { ethers } = require("hardhat"); // Not needed because it is implicitly available globally
...
describe("Token contract", function () {
    ...
    beforeEach(async function () {
      ...
      Token = await ethers.getContractFactory("WaifuCoin");
    ...
    describe("Transactions", function () {
      it("Should transfer tokens between accounts", async function () {
        // Transfer 50 * 10^-18 tokens from owner to addr1
        await hardhatToken.transfer(addr1.address, 50);
        let addr1Balance = await hardhatToken.balanceOf(addr1.address);
        expect(addr1Balance).to.equal(50);
  
        // Transfer 50 10^-18 tokens from addr1 to addr2
